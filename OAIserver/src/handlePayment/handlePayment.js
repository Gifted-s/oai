...
const handlePayment = function (req, res, User, uuidv4) {
  ...
  User.findById({ _id: _idd }, (err, user) => {
    ...
    const promise6 = paystack.initializeTransaction({
      ...
      email: user.email,
      callback_url: `https://makemyday.netlify.app/event/${_idd}`
    })
      ...
      function verifyPayment () {
        ...
        promise7.then(function (response) {
          ...
          if (response.body.data.status === 'success') {
            ...
            user.eventSchedule.amountRecieved = req.body.totalPrice + user.eventSchedule.amountRecieved
            user.eventSchedule.noOfContributors = user.eventSchedule.noOfContributors + 1
