 ...
 */
const handleUserAuth = function (req, res, User) {
  // find the user with an id and update the validMail field to true
  User.updateOne({ _id: req.params.id }, { $set: { approved: true } }, (err, response) => {
    if (err) {
  ...
  <body>
     <div class="cntainer" style="padding: 50px 30px; text-align: center;">
         ...
         <p>User now has full access to site functionalities</p>
  
         <a href="https://oneafricainitiative.netlify.app"><button class="btn btn-success" style=" font-size: 14px; color: white;font-weight: 600;">
           Go to site
         </button>
      </a>
     </div>
