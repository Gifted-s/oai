...
const handleSignin = function (req, res, User) {
  ...
  const { logEmail: email, logPass: password } = req.body
  ...
  const checkuser = function (user) {
    ...
    if (!user) {
      ...
      return res.status(200).send({ error: 'No user with this email' })
    }
    ...
    if (!user.validMail) {
      ...
      let helperfunction = {
        ...
        html: `<h1 style="color: green;font-family: Verdana;font-size: 300%;">OAI</h1>
          <p>Welcome to One Africa Initiative. Please click on the button below to confirm your email account </p>
     <a style="color:white" href="http://192.168.43.164:4000/validate-email/${user._id}"><button style="color:white;padding:20px; border:none; border-radius:5px; background:green; font-size:15px; font-weight:bold;">Confirm Email</button></a>
        `
